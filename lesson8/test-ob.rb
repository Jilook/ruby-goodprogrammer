...
puts 'Как вас зовут голубчик?'
user_name = STDIN.gets.chomp.to_s

puts 'Приятно познакомиться ' + user_name +  ' Начнем наш тест!'
...
array_answer = ['0 - нет', '1 - иногда', '2 - да']
user_answer = nil
sum_user_answer = 0


for item in array_question do
	puts item.to_s
	puts array_answer.to_s
	user_answer = STDIN.gets.chomp.to_i
	while (user_answer != 0 && user_answer != 1 && user_answer != 2)
		puts 'Выберите правильный ответ'
		user_answer = STDIN.gets.chomp.to_i
	end
	sum_user_answer = sum_user_answer + user_answer
	puts 'Сумма очков: ' + sum_user_answer.to_s
end

array_char = ['30 31 балл. Что скрывать, вы некоммуникабельны, и страдаете от этого прежде всего сами.
	...
	и вы часто сердитесь на себя за нее. Вспомните, как легко вы общаетесь, когда чем то очень увлечены.
	...
	Вы легко знакомитесь с новыми людьми, вам нравятся большие компании, особенно если вы находитесь в центре всеобщего
	внимания. Вам не помешает выработать в себе немного терпения и усидчивости, необходимых для решения серьезных проблем.',
	...
	Беретесь судить о проблемах, в которых совершенно не разбираетесь. Часто из за вас возникают конфликты.
	С серьезной работой вы справиться не в состоянии. Людям очень трудно с вами. Вам необходимо воспитывать в себе терпение,
	сдержанность и уважительное отношение к людям.']

if (sum_user_answer >= 30)
	puts array_char[0]
elsif (sum_user_answer >= 25)
	puts array_char[1]
elsif (sum_user_answer >= 19)
	puts array_char[2]
elsif (sum_user_answer >= 14)
	puts array_char[3]
elsif (sum_user_answer >= 9)
	puts array_char[4]
elsif (sum_user_answer >= 4)
	puts array_char[5]
elsif (sum_user_answer <= 3)
	puts array_char[6]
